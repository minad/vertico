...
Vertico is available from MELPA. You can install it directly via =M-x
package-install RET vertico RET=. After installation, activate the global minor
mode with =M-x vertico-mode RET=.
...
following extensions come with the Vertico package:
