...
instead. You should exist with the current input for example when you want to
create a new buffer or a new file with ~find-file~ or ~switch-to-buffer~.
...
substrings within non-remote file names, since =orderless= will stay the preferred
