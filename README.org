...
filtered on Emacs 29 or even made available for completion after entering =/ssh:=
on Emacs 28 and older. In order to avoid this problem, the =basic= completion
style should be specified for the file completion category, such that =basic= is
