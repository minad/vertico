...
(defun vertico-quick-jump ()
  ...
  (interactive)
  (let ((vertico-quick--index 0)
        (vertico-quick--list)
        (key))
    (unwind-protect
        (progn
          (advice-add #'vertico--format-candidate :around #'vertico-quick--format-candidate)
          (vertico--exhibit)
          (setq key (read-key))
      ...
      (advice-remove #'vertico--format-candidate #'vertico-quick--format-candidate))
